...
// Sources:
// Official Unity Input System Sample
// Voidloop
...
{
    public bool useControlCancel;
    public InputActionReference closeUIAction;
    public CanvasGroup duplicateCanvas;
    ...
    private IEnumerator ReenableClose()
    {
        yield return new WaitForSecondsRealtime(0.1f);

        closeUIAction.action.Enable();
    }
    ...
    private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
    {
        ...
        m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                // If use control cancel
                .WithCancelingThrough(useControlCancel && PlayerInput.all[0].currentControlScheme == "Gamepad"
                    ? "<Gamepad>/buttonEast"
                    : "<Keyboard>/escape")
                // Ignore gamepad left and right stick movement
                ...
                .OnCancel(
                    operation =>
                    {
                        ...
                        StartCoroutine(nameof(ReenableClose));
                        m_RebindOverlay?.SetActive(false);
                        UpdateBindingDisplay();
                        CleanUp();
                        duplicateCanvas.alpha = 0.0f;
                    })
                .OnComplete(
                    operation =>
                    {
    ...
    [Tooltip("Event that is triggered when an interactive rebind is complete or has been aborted.")] [SerializeField]
