...
public static class UIExtensions {
    ...
    /// <summary>
    /// Smoothly scroll the target element to the vertical center of the scroll rect's viewport.
    /// Assumes the target element is part of the scroll rect's contents.
    /// General smooth logic taken from from https://discussions.unity.com/t/how-to-have-smooth-scrolling-for-scroll-bars/746791/4
    /// </summary>
    /// <param name="scrollRect">Scroll rect to scroll</param>
    /// <param name="target">Element of the scroll rect's content to center vertically</param>
    /// <param name="duration">How long the scroll should take</param>
    public static void SmoothScrollToCenter(this ScrollRect scrollRect, RectTransform target, float duration = 0.08f)
    {
        if (scrollRect == null || target == null)
            return;

        // Get viewport (use scrollRect if no explicit viewport assigned)
        var view = scrollRect.viewport != null ? scrollRect.viewport : scrollRect.GetComponent<RectTransform>();

        // Calculate offset in viewport space
        var viewRect = view.rect;
        var elementBounds = target.TransformBoundsTo(view);
        var offset = viewRect.center.y - elementBounds.center.y;

        // Normalize and calculate target normalized position
        float targetPos = scrollRect.verticalNormalizedPosition - scrollRect.NormalizeScrollDistance(1, offset);
        targetPos = Mathf.Clamp01(targetPos);

        // Kill any ongoing tween on this ScrollRect before starting a new one
        Tween.StopAll(scrollRect);

        // Tween normalized position to the target
        Tween.UINormalizedPosition(scrollRect, new Vector2(scrollRect.normalizedPosition.x, targetPos), duration, Ease.Default, 1, CycleMode.Restart, 0, 0, true);
    }
}